class Inline::Perl5::Interpreter is repr('CPointer') {
    ...
    method p5_look_up_package_method(Str, Str) is native($p5helper)
        returns Pointer { ... }

    method p5_look_up_method(Pointer, Str) is native($p5helper)
        returns Pointer { ... }
    ...
    method p5_compile_sv(Pointer, CArray[Pointer]) is native($p5helper)
        returns int32 { ... }
